...
import { FILE_EXTENSION_REGEX, UNNAMED_BLUEPRINT_FILE_NAME } from './fileSaveAs';

export const fileExport = async () => {
  const { fileHandle } = useApp.getState();
  const fileName =
    fileHandle?.name.replace(FILE_EXTENSION_REGEX, '') ?? UNNAMED_BLUEPRINT_FILE_NAME.replace(FILE_EXTENSION_REGEX, '');
