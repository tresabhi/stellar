...
// TODO: MIGRATE TO  SEPARATE MODULES
// Constants
...
const parsedArgs = argsParser(argv);
let buildType;

if (parsedArgs.alpha) buildType = 'alpha';
else if (parsedArgs.beta) buildType = 'beta';
else if (parsedArgs.release) buildType = 'release';
...
exec('echo', async (error) => {
  if (error) {
    ...
    const newName = `${APP_NAME} ${BUILD_NAMES[parsedArgs] ?? ''}`.trim();
